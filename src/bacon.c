...
//     //     if((input_char_str[i] >= 97) && (input_char_str[i] <= 122)){
//     //        // printf("%c", input_char_str[i]);
//     //         input_char_str[i] -= 32;
//     //     }  
//     // }
...
unsigned int* char_str_to_bacon_int_str(char* input_char_str){
    ...
    toupper(input_char_str);
    int length_char_str = strlen(input_char_str);
    unsigned int* int_str_binary = (unsigned int*) malloc(6 * (length_char_str+2));
...
int encrypt(const char *plaintext, char *ciphertext) {
    ...
    // length_of_cipher /= 6;
    success_encrypt = length_of_plain;
    ...
    // {
    //     ciphertext = auto_change_case(ciphertext, plain_int_bacon, (length_of_cipher/6), (length_of_plain)*6);
    //     return success_encrypt; // need to change here
    // }
    // // if overflow
    // else{
    //     int loss_chars = length_of_plain - 1 - (length_of_cipher/6);
    //     ciphertext = auto_change_case(ciphertext, plain_int_bacon, (length_of_cipher/6), (length_of_plain)*6);
    //     success_encrypt -= loss_chars;
    // }
    free(plain_int_bacon);
...
int* read_cipher_text(char *cipher_text){
    ...
    int* int_original = (int*)malloc(sizeof(int) * length_original);
    ...
    int* int_deleted = (int*)malloc(sizeof(int) * new_length);
    int j = 0; // original
...
char* get_last_six(char* cipher){
    ...
    char *test_six_char = (char*) malloc(6* sizeof(char));
    for(int i = 0; i < 6; i++){
        test_six_char[i] = (char)(cipher[length_original - i]);
    }
...
int is_end_message(char* prob_end_message){
    ...
    for(int i = 0; i < 6; i++){
        if((int)(prob_end_message[i]) >  90){
            return 0;
        }
        if ((int)(prob_end_message[i]) < 65){
            return 0;
        }
    }
    return 1;
...
// conbine the 2 functions above, 
int test_last_six(char* test_string){
    char* last_six_char = get_last_six(test_string);
    return (is_end_message(last_six_char));
}
...
int decrypt(const char *ciphertext, char *plaintext) {
    ...
    // // reconstruct the cipher text here, delete anything except english  chars
    // char* new_string[cipher_length];
    // int j = 0;
    // for(int i = 0; i < cipher_length; i++){
    //     if(is_english_char(ciphertext[j]) == 1){
    //         // new_string[i] = (char)(ciphertext[j]);
    //     }
    //     j++;
    // }
    // ciphertext = (char*)new_string;
    int result_counter = 0;
    int *decimal_results [cipher_length / 6];
    cipher_int = read_cipher_text((char*)ciphertext);
    ...
    for(int i = 0; i < plain_length; i++){
        plaintext[i] = (int)get_char_from_bacon(*decimal_results[i]);
    }
